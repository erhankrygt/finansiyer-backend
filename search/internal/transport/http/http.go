...
import (
	...
	"reflect"
	"search"
	"search/internal/endpoints"
	trans "search/internal/transport"
)
...
func MakeHTTPHandler(l log.Logger, s search.Service) http.Handler {
	...
	// services docs
	// swagger router
	swaggerRouter := searchRouter.PathPrefix("/docs").Subrouter()
...
func makeHealthHandler(e endpoint.Endpoint, serverOptions []kithttp.ServerOption) http.Handler {
	h := kithttp.NewServer(e, makeDecoder(search.HealthRequest{}), encoder, serverOptions...)
...
func makeDefaultServerOptions(l log.Logger, endpointName string) []kithttp.ServerOption {
	return []kithttp.ServerOption{
		kithttp.ServerErrorHandler(trans.NewErrorHandler(l, endpointName)),
	}
}
